...
public class AccesoController {
	...
	@PostMapping
	public ResponseEntity<Acceso>create(@Valid @RequestBody Acceso ace){
		try {
			Acceso a=accesoService.create(ace);
	...
	@PutMapping("/{id}")
	public ResponseEntity<Acceso>updateAcceso(@PathVariable("id") Long id, @Valid @RequestBody Acceso ace){
		Acceso a=accesoService.read(id);
		if (a.getId()>0) {
			return new ResponseEntity<>(accesoService.update(ace),HttpStatus.OK);
		} else {
			return new ResponseEntity<>(HttpStatus.NO_CONTENT);
		}
	}
